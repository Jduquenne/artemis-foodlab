...
1. `core/domain/types.ts` — domain types (`Recipe`, `Ingredient`, `Food`, `Unit` enum, `IngredientCategory` enum, etc.). Note: `IngredientCategory` has **three separate spice/condiment values** — `CONDIMENT`, `SPICE`, `AROMATIC_HERB` — there is no longer a combined `SPICE_CONDIMENT` value.
...
Current version: **6.31.0**. The next commit must update `package.json` accordingly before the commit message is given.
