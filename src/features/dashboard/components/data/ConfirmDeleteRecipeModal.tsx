...
export interface ConfirmDeleteRecipeModalProps {
  recipe: RecipeDetails;
  onCancel: () => void;
  onConfirm: (code: string) => Promise<boolean>;
}

export const ConfirmDeleteRecipeModal = ({ recipe, onCancel, onConfirm }: ConfirmDeleteRecipeModalProps) => {
  const [submitting, setSubmitting] = useState(false);

  const confirm = async () => {
    setSubmitting(true);
    const ok = await onConfirm(recipe.code);
    setSubmitting(false);
    if (ok) onCancel();
  };
    ...
    <div className="fixed inset-0 z-100 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-100 w-full max-w-sm rounded-t-3xl sm:rounded-3xl shadow-2xl p-5 flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          <h2 className="text-lg font-black text-slate-900">Supprimer cette recette ?</h2>
          <p className="text-sm text-slate-500">
            <span className="font-semibold text-slate-700">{recipe.name}</span> sera retirée du catalogue. Si un
