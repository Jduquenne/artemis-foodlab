...
export interface FoodFormModalProps {
  food: Food;
  onClose: () => void;
  onSubmit: (id: string, body: FoodInput) => Promise<boolean>;
}
...
export const FoodFormModal = ({ food, onClose, onSubmit }: FoodFormModalProps) => {
  const [draft, setDraft] = useState<FoodFormDraft>(() => foodToDraft(food));
  ...
  const submit = async () => {
    ...
    setErrors([]);
    setSubmitting(true);
    const ok = await onSubmit(food.id, foodFormToBody(food.id, draft));
    setSubmitting(false);
    if (ok) onClose();
  };
    ...
    <div className="fixed inset-0 z-100 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-100 w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden max-h-[90dvh]">
        <div className="p-5 border-b border-slate-200 flex justify-between items-center shrink-0">
          <div>
            <h2 className="text-lg font-black text-slate-900">Modifier l'aliment</h2>
            <p className="text-xs text-slate-400">{food.id}</p>
          </div>
