...
export const IngredientBuilderList = ({ ingredients, onChange }: IngredientBuilderListProps) => {
  ...
  const editingIngredient = editingId ? ingredients.find(i => i.id === editingId) ?? null : null;
    ...
    <>
        ...
        <div className="flex items-center justify-between mb-2 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={add}
              className="p-1 rounded-lg text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors"
      ...
      {editingIngredient && (
        <IngredientEditDrawer
          ...
          onChange={updated => update(editingIngredient.id, updated)}
